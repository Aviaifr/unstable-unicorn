...
export function returnGameData(
  ...
  if (!room || !player) {
    ...
    if (game) {
      ...
      io?.sockets.adapter.rooms
        .get(`room_${room.uuid}`)
        ?.forEach((clientId) => {
          const sid = (io?.sockets.sockets.get(clientId)?.handshake as any)
            ?.sessionID;
          io?.to(clientId).emit(
            "game_started",
            game.toJson(getSessionPlayer(sid))
          );
        });
    }
  }
...
function getSessionGameObjects(
  socket: Socket<DefaultEventsMap, DefaultEventsMap, DefaultEventsMap>
...
export function playCard(
  socket: Socket<DefaultEventsMap, DefaultEventsMap, DefaultEventsMap>,
  playData: { target: string; targetPlayer: string; card: string }
): void {
